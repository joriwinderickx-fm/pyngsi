...
class SinkHttp(Sink):
    ...
    def __init__(self, hostname="127.0.0.1", port=8080, secure=False, baseurl="/",
        ...
        self.prefix = f"{self.protocol}://{hostname}:{port}{baseurl}"
        self.post_url = f"{self.prefix}{post_endpoint}?{post_query}" if post_query else f"{self.prefix}{post_endpoint}"
        self.status_url = f"{self.prefix}{status_endpoint}"
        self.proxy = proxy
